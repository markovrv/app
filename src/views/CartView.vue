    ...
    <div v-if="count>0">
          ...
          <table class="table table-bordered">
              ...
              <tbody>
                  <tr v-for="good, id in cart" :key="id">
                      ...
                      <td>{{good.price}}</td>
                      <td>
                        <input :value="good.count" @change="(el) => {changeCount(el, good)}" type="number" min="1" class="form-control form-control-sm text-center">
                      </td>
      ...
      <div>
          <div class="form-group row">
            ...
            <input v-model="name" type="text" id="name" class="form-control col-sm-6">
          </div>
          <div class="form-group row">
          ...
          <div class="form-group row">
            <label for="info" class="col-sm-4 text-sm-right" style="margin-top: 5px;">
              ...
              <textarea v-model="info" type="text" id="info" rows="3" class="form-control col-sm-6"></textarea>
          </div>
      </div>
      <div>
        <button @click="sendCart" type="button" class="btn btn-success btn-lg">
...
export default {
  ...
  methods: {
    changeCount(el, good) {
      console.log(el)
      if (Number(el.target.value) > 0 && Number(el.target.value) < 100)
        good.count = Number(el.target.value)
    },
    delItem(id) {
    ...
    sendCart() {
      ...
      this.axios.post('http://195.161.62.172:7777/markov.rv/api/orders.php', data)
        .then(resp => {
